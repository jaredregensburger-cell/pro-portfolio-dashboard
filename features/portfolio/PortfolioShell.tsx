    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <GlassCard accent="gain" className="space-y-3">
          ...
          <p className={cn('font-mono text-data-3xl font-semibold tracking-tight', gainColor(unrealizedPnL.amount))}>
            {unrealizedPnL.amount >= 0 ? '+' : ''}
            {formatCurrency(unrealizedPnL.amount)}
          </p>
          <p className="text-data-sm text-ink-muted">
            vs. cost basis of {formatCurrency(totalCost)}
          </p>
        </GlassCard>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        {/* Allocation Donut */}
        <GlassCard className="lg:col-span-1 flex flex-col items-center justify-center min-h-[280px]" padding="sm">
          <p className="text-data-sm font-semibold text-ink self-start mb-2">Allocation</p>
                ...
                return (
                  <div key={slice.assetClass} className="flex items-center gap-4 px-5 py-3.5">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-data-sm font-medium text-ink">{meta.label}</p>
                    </div>
